...
df = pd.read_csv( folder + os.sep + 'TinySOL_metadata.csv' )
...
for c in categories:
    ...
    for p in paths:
        print( p )
        s, _ = librosa.load( folder + p, sr=16000 )
        # keep 1 sec
        tiny_SOL_A2[c].append( s[:16000] )
        tiny_SOL_A234[c].append( s[:16000] )
